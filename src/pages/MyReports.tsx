...
export default function MyReports() {
  ...
  useEffect(() => {
    Promise.all([api.getItems('lost'), api.getItems('found')]).then(([lost, found]) => {
      ...
      if (user?.id) {
        const userSpecific = combined.filter(item => item.userId === user.id);
        setItems(userSpecific.length > 0 ? userSpecific : combined);
      } else {
        setItems(combined);
      }
      setLoading(false);
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
          <div className="divide-y divide-slate-100">
            {items.map(item => (
              <div key={item.id} className="p-6 flex flex-col sm:flex-row gap-6 items-center hover:bg-slate-50 transition-colors">
